...
public class DepartmentDAOTest {
    ...
    void testSimpleManipulations() {
        List<Department> departmentListAll = (List<Department>) departmentDAO.getAllDepartments();
        assertEquals(5, departmentListAll.size());

        List<Department> departmentList = departmentDAO.getAllDepartmentsByName("продаж");
        assertEquals(1, departmentList.size());
        assertEquals("Отдел продаж", departmentList.get(0).getName());

        List<Department> subDepsList = departmentDAO.getAllSubdepartments("Главное отделение");
        List<String> subs = new ArrayList<>();
        for (int i = 0; i < subDepsList.size(); i++) {
            subs.add(subDepsList.get(i).getName());
        }
        Collections.sort(subs);
        List<String> s = new ArrayList<>();
        s.add("Отдел продаж");
        s.add("Технический отдел");

        assertEquals(s, subs);

        Department d = departmentDAO.getSingleDepartmentByName("Технический отдел");
        assertEquals("Петров Виктор Сергеевич", d.getWorker().getName());
    }

    @Test
    void testUpdate() {
        Worker w = workerDAO.getById(9L);
        Department d = departmentDAO.getSingleDepartmentByName("Отдел системного администрирования");
        assertNotNull(d);

        d.setWorker(w);
        departmentDAO.update(d);

        d = departmentDAO.getSingleDepartmentByName("Отдел системного администрирования");
