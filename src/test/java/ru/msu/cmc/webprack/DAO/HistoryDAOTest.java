...
public class HistoryDAOTest {
    ...
    void testSimpleManipulations() {
        List<History> h;
        h = historyDAO.getWorkerHistory(1L);
        assertEquals(2, h.size());
    }

    @Test
    void testUpdate() {
        History newH = new History(null, workerDAO.getById(1L), positionDAO.getById(3L), Timestamp.valueOf("2022-02-02 00:00:00"), null);
        History oldH = historyDAO.getById(1L);
        oldH.setFinish(Timestamp.valueOf("2022-02-01 00:00:00"));
        historyDAO.update(oldH);
        List<History> historyList = historyDAO.getWorkerHistory(1L);
        assertEquals(2, historyList.size());
        historyDAO.save(newH);
        historyList = historyDAO.getWorkerHistory(1L);
